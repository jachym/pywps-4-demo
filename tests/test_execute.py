...
from tests.common import validate, URL, get_response, PY2
...
class SayHello(unittest.TestCase):
    ...
    def test_valid(self):
        ...
        url = URL + '?service=wps&request=execute&'\
                    'identifier=say_hello&version=1.0.0&datainputs=name=ahoj'
        assert validate(url, self.schema_url)

    def test_valid_lineage(self):
        "GET Execute request, lineage=true"

        url = URL + '?service=wps&request=execute&identifier=say_hello&'\
                    'version=1.0.0&datainputs=name=ahoj&lineage=true'
        assert validate(url, self.schema_url)
...
class Buffer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        resp = get_response('http://localhost:5000/static/'\
                            'requests/execute_buffer_post.xml')
...
def load_tests(loader=None, tests=None, pattern=None):
    ...
    if PY2:
        suite_list.append(loader.loadTestsFromTestCase(Warp))
